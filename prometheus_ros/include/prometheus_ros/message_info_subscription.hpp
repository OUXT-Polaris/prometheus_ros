...
#ifndef PROMETHEUS_ROS__MESSAGE_INFO_SUBSCRIPTION_HPP_
#define PROMETHEUS_ROS__MESSAGE_INFO_SUBSCRIPTION_HPP_
 ...
 */
class MessageInfoSubscription : public rclcpp::SubscriptionBase
{
public:
  // cppcheck-suppress unknownMacro
  RCLCPP_SMART_PTR_DEFINITIONS(MessageInfoSubscription)
  ...
  : SubscriptionBase(
      ...
      topic_name, options.template to_rcl_subscription_options<rclcpp::SerializedMessage>(qos),
      true),
    callback_(callback),
    ts_lib_(ts_lib)
  {
    ...
    if (options.event_callbacks.deadline_callback) {
      this->add_event_handler(
        options.event_callbacks.deadline_callback, RCL_SUBSCRIPTION_REQUESTED_DEADLINE_MISSED);
    }
    if (options.event_callbacks.liveliness_callback) {
      this->add_event_handler(
        options.event_callbacks.liveliness_callback, RCL_SUBSCRIPTION_LIVELINESS_CHANGED);
    }
    ...
    if (options.event_callbacks.message_lost_callback) {
      this->add_event_handler(
        options.event_callbacks.message_lost_callback, RCL_SUBSCRIPTION_MESSAGE_LOST);
    }
  }

  RCLCPP_PUBLIC
  virtual ~MessageInfoSubscription() = default;
...
#endif  //PROMETHEUS_ROS__MESSAGE_INFO_SUBSCRIPTION_HPP_
